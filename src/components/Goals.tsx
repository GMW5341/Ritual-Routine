    ...
    <div className="space-y-8">
          ...
          {TERM_TABS.map((tab) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                activeTerm === tab.key ? 'bg-emerald-500 text-white' : 'text-white/50 hover:text-white/80'
              }`}
            >
              {tab.label}
      ...
      {showForm && (
        <div className="bg-white/[0.03] border border-white/10 rounded-xl p-5 space-y-4">
          <h3 className="text-sm font-semibold text-white/80">새 목표 추가</h3>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="text-[10px] text-white/40 uppercase tracking-wider">목표 *</label>
              <input
                type="text"
                value={formTitle}
            ...
            <div>
              <label className="text-[10px] text-white/40 uppercase tracking-wider">세부 설명</label>
              <textarea
                value={formDesc}
                onChange={(e) => setFormDesc(e.target.value)}
                placeholder="구체적인 계획이나 방법 (선택)"
                rows={2}
                className="w-full mt-1 bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-sm text-white/80 placeholder-white/20 focus:outline-none focus:border-emerald-500/30 focus:ring-1 focus:ring-emerald-500/30 resize-none"
              />
            </div>
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="text-[10px] text-white/40 uppercase tracking-wider">기간</label>
                <select
                  value={formTerm}
              ...
              <div className="flex-1">
                <label className="text-[10px] text-white/40 uppercase tracking-wider">목표일</label>
                <input
                  type="date"
                  value={formDeadline}
                  onChange={(e) => setFormDeadline(e.target.value)}
                  className="w-full mt-1 bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-sm text-white/80 focus:outline-none focus:border-emerald-500/30 [color-scheme:dark]"
                />
              </div>
            </div>
      ...
      {filteredGoals.length > 0 ? (
        ...
        <div className="text-center py-16 text-white/20">
          ...
          <p className="text-xs mt-1 text-white/15">위의 &quot;새 목표&quot; 버튼으로 추가해보세요.</p>
        </div>
      )}
    </div>
  );
...
function GoalCard({
  ...
  return (
    <div className={`bg-white/[0.02] border rounded-xl p-4 transition-all ${
      isCompleted ? 'border-emerald-500/10 opacity-60' : isDropped ? 'border-white/5 opacity-40' : 'border-white/5'
    }`}>
      {isEditing ? (
        <div className="space-y-2">
          <input
            ...
            className="w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-sm text-white/80 focus:outline-none focus:border-emerald-500/30"
            autoFocus
          />
          <textarea
            value={editDesc}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className={`text-sm font-semibold ${isCompleted ? 'text-white/40 line-through' : 'text-white/80'}`}>
                  {goal.title}
                </h3>
                ...
                {deadlineStr && (
                  <span className={`text-[9px] ${isOverdue ? 'text-red-400' : 'text-white/25'}`}>
              ...
              {goal.description && (
                <p className="text-xs text-white/35 mt-1 leading-relaxed whitespace-pre-wrap">{goal.description}</p>
              )}
              ...
              {goal.status === 'active' && (
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <span className="text-[10px] text-white/30 shrink-0 w-8 text-right">{goal.progress}%</span>
                </div>
              )}
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={onStartEdit} className="p-1 text-white/20 hover:text-white/50 transition-colors">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {confirmDelete ? (
                ...
                <button onClick={() => setConfirmDelete(true)} className="p-1 text-white/20 hover:text-red-400 transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
