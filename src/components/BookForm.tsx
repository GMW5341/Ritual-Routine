  ...
  return (
    <div className="bg-[#272c38]/50 border border-[#313744]/60 rounded-xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white/85">새 책 추가</h3>
        <button onClick={onClose} className="p-1 text-white/60 hover:text-white/65 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label className="text-[10px] text-white/70 uppercase tracking-wider">제목 *</label>
          <input
            type="text"
            value={title}
        ...
        <div>
          <label className="text-[10px] text-white/70 uppercase tracking-wider">저자</label>
          <input
            type="text"
            value={author}
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="text-[10px] text-white/70 uppercase tracking-wider">총 페이지</label>
            <input
              type="number"
              value={totalPages}
          ...
          <div className="flex-1">
            <label className="text-[10px] text-white/70 uppercase tracking-wider">상태</label>
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as ReadingStatus)}
              className="w-full mt-1 bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500/30 focus:ring-1 focus:ring-emerald-500/30"
            >
              {STATUS_OPTIONS.map((opt) => (
                <option key={opt.value} value={opt.value} className="bg-[#272c38]">
                  {opt.label}
                </option>
              ))}
            </select>
